...
/**
 * Concrete SnapDB client implementation.
 */
public class SnapDBClient extends DB {
  static final String PROPERTY_SCAN_VALUE = "scan.value";
  static final String PROPERTY_SCAN_DIST = "scan.dist";
  private Logger logger = LoggerFactory.getLogger(getClass());

  /**
   * The SnapDBClient implementation that will be used to communicate
   * with the SnapDB server.
   */
  private static SnapDB client;
  ...
  public void init() throws DBException {
    synchronized(SnapDBClient.class) {
      if(client == null) {
        ...
        client = new SnapDB();
        System.out.printf("initial SnapDB\n");
        try {
          client.init();
  ...
  public void cleanup() throws DBException {
    synchronized(SnapDBClient.class) {
      if (references == 1) {
        client.close();
        System.out.printf("SnapDB cleanup\n");
      }
      references--;
    }
  }
